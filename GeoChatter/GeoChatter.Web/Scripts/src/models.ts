    ...
    implements IParentedBy<Round>, ICountrySource, IdentifiedBy<PlayerData, Guess>
{
    ...
    AsTableRow = function(this: Guess) : TableRow
    {
        ...
        if (this.Round.Game.Mode == Enum.GAMEMODE.DEFAULT)
        {
            ...
            row.CountryStreak = this.Round.Game.Stage == Enum.GAMESTAGE.INROUND ? Util.AsDataTableRowCell("-", -1) : Util.AsDataTableRowCell(this.CountryStreak.toString(), this.CountryStreak);
            row.Distance = this.Round.Game.Stage == Enum.GAMESTAGE.INROUND ? Util.AsDataTableRowCell("-", 999999) : Util.GetConvertedDistance(this.Distance);
            row.Score = this.Round.Game.Stage == Enum.GAMESTAGE.INROUND ? Util.AsDataTableRowCell("-", -1) : Util.AsDataTableRowCell(this.Score.toString(), this.Score);
